...
export class SharedUtilFormattersService {
  ...
  dateFormatter(
    ...
    // Avoid creating the 'format' object and using spread (O(N) allocation) per call.
    // Hoisting default values and accessing 'extras()' result directly is faster.
    const extraValues = extras?.();
    const dateDigitsInfo = extraValues?.dateDigitsInfo ?? 'shortDate';
    const locale = extraValues?.locale ?? this.#locale;
    const timezone = extraValues?.timezone ?? this.#timezone;
  ...
  percentageFormatter(
    ...
    const extraValues = extras?.();
    const numberDigitsInfo = extraValues?.numberDigitsInfo ?? '1.2-2';
    const locale = extraValues?.locale ?? this.#locale;
  ...
  currencyFormatter(
    ...
    const numberDigitsInfo = extraValues?.numberDigitsInfo ?? '1.2-2';
    const locale = extraValues?.locale ?? this.#locale;
    const currency = extraValues?.currency ?? '€';
    const currencyCode = extraValues?.currencyCode ?? 'EUR';
  ...
  quantityFormatter<T extends BaseEntity>(
    ...
    const extraValues = extras?.(item);
    const numberDigitsInfo = extraValues?.numberDigitsInfo ?? '1.2-2';
    const locale = extraValues?.locale ?? this.#locale;
    const suffix = extraValues?.suffix ?? '';
    const prefix = extraValues?.prefix ?? '';

    const formattedNumber = formatNumber(Number(value), locale, numberDigitsInfo);
  ...
  booleanWithIconFormatter(
    ...
    const extraValues = extras?.();
    const iconTrue = extraValues?.iconTrue ?? 'check';
    const iconFalse = extraValues?.iconFalse ?? 'close';

    return this.#sanitizer.bypassSecurityTrustHtml(
      `<span class="material-icons">${escapeHtml(value ? iconTrue : iconFalse)}</span>`
    );
  }
