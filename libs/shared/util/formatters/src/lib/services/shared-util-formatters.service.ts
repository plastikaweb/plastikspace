...
export class SharedUtilFormattersService {
  ...
  dateFormatter(
    ...
    const extraOpts = extras?.();
    const dateDigitsInfo = extraOpts?.dateDigitsInfo ?? 'shortDate';
    const locale = extraOpts?.locale ?? this.#locale;
    const timezone = extraOpts?.timezone ?? this.#timezone;
  ...
  firebaseTimestampFormatter(
    ...
    return formatDate(value?.toDate(), dateDigitsInfo, locale, timezone);
  }
  ...
  percentageFormatter(
    ...
    const extraOpts = extras?.();
    const numberDigitsInfo = extraOpts?.numberDigitsInfo ?? '1.2-2';
    const locale = extraOpts?.locale ?? this.#locale;
  ...
  quantityFormatter<T extends BaseEntity>(
    ...
    const extraOpts = extras?.(item);
    const numberDigitsInfo = extraOpts?.numberDigitsInfo ?? '1.2-2';
    const locale = extraOpts?.locale ?? this.#locale;
    const prefix = extraOpts?.prefix ?? '';
    const suffix = extraOpts?.suffix ?? '';
  ...
  booleanWithIconFormatter(
    ...
    const extraOpts = extras?.();
    const iconTrue = extraOpts?.iconTrue ?? 'check';
    const iconFalse = extraOpts?.iconFalse ?? 'close';
