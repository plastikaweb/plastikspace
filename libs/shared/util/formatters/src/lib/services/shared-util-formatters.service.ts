...
export class SharedUtilFormattersService {
  ...
  dateFormatter(
    ...
    const ext = extras?.();
    const dateDigitsInfo = ext?.dateDigitsInfo ?? 'shortDate';
    const locale = ext?.locale ?? this.#locale;
    const timezone = ext?.timezone ?? this.#timezone;
  ...
  percentageFormatter(
    ...
    const ext = extras?.();
    const numberDigitsInfo = ext?.numberDigitsInfo ?? '1.2-2';
    const locale = ext?.locale ?? this.#locale;
  ...
  currencyFormatter(
    ...
    const numberDigitsInfo = ext?.numberDigitsInfo ?? '1.2-2';
    const locale = ext?.locale ?? this.#locale;
    const currency = ext?.currency ?? '€';
    const currencyCode = ext?.currencyCode ?? 'EUR';
  ...
  quantityFormatter<T extends BaseEntity>(
    ...
    const numberDigitsInfo = ext?.numberDigitsInfo ?? '1.2-2';
    const locale = ext?.locale ?? this.#locale;
    const prefix = ext?.prefix ?? '';
    const suffix = ext?.suffix ?? '';
  ...
  booleanWithIconFormatter(
    ...
    const ext = extras?.();
    const iconTrue = ext?.iconTrue ?? 'check';
    const iconFalse = ext?.iconFalse ?? 'close';
