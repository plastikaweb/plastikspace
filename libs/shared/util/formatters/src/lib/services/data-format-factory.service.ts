...
export class DataFormatFactoryService<T extends FormattingInput<keyof T> & BaseEntity> {
   ...
   * Uses a fast-path for non-nested keys, a `Map`-based cache for split path
   * parts, and a manual loop instead of `reduce()` to avoid O(N) array allocation
   * and closure overhead on every call.
  ...
  #getValueFromRow(property: string, item: T extends BaseEntity ? T : never): FormattingOutput {
    ...
    for (let i = 0; i < parts.length; i++) {
      acc = (acc as T)[parts[i] as keyof T];
      if (isNil(acc)) {
        return '';
      }
    }

    return acc as FormattingOutput;
  }
}
