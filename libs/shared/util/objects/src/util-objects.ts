...
/**
 * @description Returns an object without properties with null value. Optimized to use an
 * imperative `for...in` loop to eliminate redundant O(N) intermediate array allocations
 * from `Object.entries()`, reducing memory pressure and improving performance.
 ...
 * Optimized to use an imperative `for...in` loop to eliminate redundant O(N) intermediate
 * array allocations from `Object.entries()`, reducing memory pressure and improving performance.
 * @param  {Record<string, string | number | boolean | null>} collection Object parameter passed.
