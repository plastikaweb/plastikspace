...
export type StoreFirebaseCrudFeature<
  ...
  C extends FirebaseCrudState<T, F>,
> = {
  featureName: string;
  dataServiceType: Type<S>;
  baseRoute?: FirebaseCrudState<T, F>['baseRoute'];
  initState: C;
};
 ...
 * @param {FirebaseCrudPagination<T>} [pagination] The initial pagination state with pageIndex=0, pageSize=10 and empty pageLastElements.
 * @param {TableSortingConfig} [sorting] The initial sorting state.
 * @param {FirebaseCrudState<T, F>['baseRoute']} [baseRoute] The base route for the feature used to navigate to the entity list, f.e. after creating or updating an entity.
 * @param {FirebaseCrudState<T, F>['_adminOnly']} [_adminOnly] The initial admin only state.
 * @param {FirebaseCrudState<T, F>['_public']} [_public] The initial public state.
 * @returns {FirebaseCrudState<T, F>} The initial state for the signal store feature.
 */
export function initStoreFirebaseCrudState<T extends BaseEntity, F extends FirebaseCrudFilter>(
  filter: F = {} as F,
  ...
  baseRoute: FirebaseCrudState<T, F>['baseRoute'] = '',
  _adminOnly: FirebaseCrudState<T, F>['_adminOnly'] = true,
  _public: FirebaseCrudState<T, F>['_public'] = false
...
>({ featureName, dataServiceType, initState }: StoreFirebaseCrudFeature<T, S, F, C>) {
  return signalStoreFeature(
    ...
    withState<FirebaseCrudState<T, F>>(
    ...
    withMethods(store => {
      ...
      return {
        ...
        setPagination: (pagination: Pick<FirebaseCrudPagination<T>, 'pageIndex' | 'pageSize'>) => {
          const newPagination = {
        ...
        resetTableConfig: (
          pagination: Pick<FirebaseCrudPagination<T>, 'pageSize' | 'pageIndex'>,
          filter: F,
        ...
        getItem: rxMethod<IdType<T>>(
          pipe(
            filter(() => store._activeConnection()),
            tap(() => store._activityStore.setActivity(true)),
            switchMap(id => {
              return store._dataService.getOne(id).pipe(
                tapResponse({
                  next: item => {
                    if (!item) {
        ...
        update: rxMethod<{ item: Partial<T>; redirectUrl?: string }>(
          pipe(
            ...
            tap(() => store._activityStore.setActivity(true)),
            switchMap(({ item, redirectUrl }) => {
              return store._dataService.update(item.id as IdType<T>, item).pipe(
                tapResponse({
                  next: () => {
                    const baseRoute = store.baseRoute();
                    const route =
        ...
        delete: rxMethod<IdType<T>>(
          pipe(
            ...
            tap(() => store._activityStore.setActivity(true)),
            switchMap(id => {
              return store._dataService.delete(id).pipe(
                tapResponse({
                  next: () => {
                    if (store.showNotification()) {
