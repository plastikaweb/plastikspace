...
export const llecoopUserOrderCartStore = signalStore(
  ...
  withComputed(({ cart }) => ({
    getCartTotalPrice: computed(() =>
      cart().reduce(
        (acc, cartItem) => Number((acc + cartItem.priceWithIva * cartItem.quantity).toFixed(2)),
        0
      )
    ),
    getOrderedCartItems: computed(() =>
      [...cart()].sort((itemA, itemB) => itemA.name.localeCompare(itemB.name))
    ),
  })),
  withMethods(store => {
    return {
          ...
          map(({ quantity, ...product }) => {
            ...
            updateState(store, `[user-order-cart] add item`, {
              cart:
                quantity === 0
                  ? currentCart
                  : [...currentCart, { ...product, quantity: Number(quantity) }],
            });
          })
        )
      ),
