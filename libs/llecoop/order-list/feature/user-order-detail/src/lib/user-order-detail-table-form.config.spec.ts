...
describe('LlecoopUserOrderDetailFormTableConfig', () => {
  ...
  it('should escape XSS in unit.base for priceWithIva column', () => {
    const tableDef = runInInjectionContext(injector, () => service.getTableDefinition());
    const priceColumn = tableDef.columnProperties().find(c => c.key === 'priceWithIva');
    const execute = priceColumn?.formatting?.execute;
