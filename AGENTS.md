...
- **Manual Population:** Use `yarn eco-store:pb:populate` to download the binary, sync the schema,
  and seed local data from staging in a single step.
- **Seeding:** Use `yarn eco-store:pb:seed` to specifically pull data and images from the staging environment to local.
- **Exporting:** Use `yarn eco-store:pb:export` to export the current local schema and data (JSON) for manual backups. Note: the `data/` folder is git-ignored to keep the repo lean.
