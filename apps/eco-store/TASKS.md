...
| **TECH-13** _(new)_ | Optimize `libs/shared/util/formatters` hot path: `#getValueFromRow` runs `split('.').reduce()` per table cell — add a fast-path for simple keys, a `Map` cache for split nested paths and a manual loop; drop the per-call config-object spreads in `SharedUtilFormattersService` (direct access + nullish coalescing). Behaviour-identical; sibling to TECH-07/11. Harvest ref Jules #1217 (dupes #1203/#1205/#1211/#1214/#1223 closed)                                                                                                                                                                                                                                                       | COULD    | `86cajqpt6` |
| **TECH-14** _(new)_ | Optimize `isEmpty` (`shared/util/objects`) to an early-return `for…in` and adopt it at the `Object.keys().length > 0` call sites (`eco-store-tenant.store/utils`, `with-pocketbase-params.store`) — the checks allocate the full key array per emptiness test. Sibling to TECH-03/11. Harvest ref Jules #1219 (dupe #1210 closed; its formatter-file changes belong to TECH-13)                                                                                                                                                                                                                                                                                                                | COULD    | `86cajqptd` |
