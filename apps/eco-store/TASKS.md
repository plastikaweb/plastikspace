...
| **TECH-08** _(new)_ | Optimize `BytesToSizePipe` (`libs/shared/util/bytes-to-size`) — cache `Math.log(1024)`, drop `parseInt(String(…))` round-trip, optionally precompute 1024 powers (PR #1155)            | COULD    | `86cac31nm` |
| **TECH-09** _(new)_ | `latinize()` ASCII fast-path early-return (`libs/shared/util/latinize`) — skip `.replace()` + map lookup for pure-ASCII input; **distinct** from TECH-04's regex narrowing (PR #1163)  | COULD    | `86cac31p6` |
...
| **SEC-04** _(new)_ | Harden `NotificationUiMatSnackbar` — swap `[innerHTML]="data.message"` for `{{ }}` interpolation. **Defense-in-depth only** (Angular auto-sanitizes plain `[innerHTML]`; `message` is pre-translated by `StoreNotificationService`). Deferred "optional extra" of SEC-03 — never shipped — LOW (PR #1161) | COULD      | `86cac31md` |
