...
- **A11Y-008** 📋 — `SharedAlertUiComponent` (`libs/shared/alert/ui`): add `matTooltip` to the close icon button mirroring its `common.close` `aria-label` (sibling to A11Y-004/005/007; close button has the label but no hover hint). Spec asserts tooltip↔aria-label sync. Harvest from Jules #1182. [SHOULD · 📋 · ClickUp `86cadtkh7`]
- **A11Y-009** 📋 — `textarea-with-counter` (`libs/shared/form/ui/textarea-with-counter`): add `aria-live="polite"` to the character-count hint (SR announces updates) + a `text-warning` state at ~90% of `maxLength` before the existing `text-error` at the limit. Spec asserts threshold + aria-live. Harvest from Jules #1185. [SHOULD · 📋 · ClickUp `86cadtkvb`]
...
| **TECH-07** _(new)_ | Cache system timezone in `SharedUtilFormattersService` — `Intl…resolvedOptions().timeZone` recomputed inline per call in 3 date formatters; cache once as `#timezone` field (PR #1147)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | COULD    | `86ca8m6aj` |
...
| **TECH-11** _(new)_ | Optimize `deepClone` + `escapeHtml` in `libs/shared/util/objects` — `deepClone` swaps `obj.map()`/`Object.keys().forEach()` for manual `for`/`for…in` loops; `escapeHtml` (SEC-01 escaper) hoists its `map`+regex to module scope and adds a `RegExp.test()` fast-path (return input unchanged when no special chars). Behaviour-identical. Distinct from TECH-03/05/06. Harvest from Jules #1183                                                                                                                                                                                                                                                                                              | COULD    | `86cadtm4h` |
