...
| **SEC-06** _(new)_ | XSS: escape dynamic values in **llecoop** list-facade `.confirm()` dialog **messages** — 6 facades (category/order-list ×3/product/user + `user-feature-table.config`) interpolate record data (names, user **emails**) into the message string; `SharedConfirmFeatureComponent` passes it through `translate.instant()` + `bypassSecurityTrustHtml`, and SEC-03 only escaped the _params_, never the message. A registered user controls their own email → executes in the admin's session — HIGH. Harvest ref Jules #1207 (dupes #1204/#1209 closed)                                                                                                                                                                                                                                                                                                                                                                                | MUST       | `86cajqprd` |
| **SEC-07** _(new)_ | XSS: `SharedConfirmFeatureComponent` non-string translate params (SEC-03 residual) — the `typeof value === 'string'` guard lets object/array params through unescaped; ngx-translate stringifies them into the `bypassSecurityTrustHtml` output. Fix: `escapeHtml(String(value ?? ''))` for all params, drop the guard; spec with an array-wrapped payload — MEDIUM. Harvest ref Jules #1218 (dupe #1212 closed)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | SHOULD     | `86cajqprk` |
| **SEC-08** _(new)_ | Escape `LlecoopProductBaseUnitTextPipe` output before `bypassSecurityTrustHtml` in `user-order-detail-table-form.config` + `user-order-feature-resume-table.config` (SEC-05 residual — its spec listed `${unit}` as a sink but the shipped fix left the pipe output raw; pipe interpolates `unit.base`, typed number, so LOW defense-in-depth). Harvest ref Jules #1220                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | COULD      | `86cajqprw` |
