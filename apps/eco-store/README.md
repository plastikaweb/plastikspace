...
1. **Intelligent Setup**: `yarn install:local`
   - Automatically cleans the workspace and installs dependencies.
   - Downloads the PocketBase binary if it's missing.
   - Interactively prompts to populate the local database (schema + staging data) if no existing data is found.
