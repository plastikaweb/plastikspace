...
- **`apps/eco-admin` + `apps/eco-admin-e2e`**: the seventh application in the workspace, a CSR SPA (REQUIREMENTS §6 — _"SPA, no SSR (internal tool; no SEO)"_), generated with `@nx/angular:application` on the `@angular/build:application` builder. Serves on **port 4204** (4200-4203 were taken by eco-store, llecoop, nasa-images and plastikaweb). Prefix `eco-admin`, `lang="ca"`, tagged `scope:eco-admin` + `type:app`. Two generator defaults had to be overridden by hand: `nx.json` still declares `unitTestRunner: "vitest"`, which is **not a valid value** in `@nx/angular` v22.7.1 (the choices are `vitest-angular`, `vitest-analog`, `jest`, `none`), so the app and every lib were generated with an explicit `--unitTestRunner=vitest-analog` — the one that yields the `@nx/vitest:test` target the other four apps and 180 libs use — and `@nx/angular:library` did **not** inherit `style: scss` from `nx.json` (only `:application` and `:component` declared it), so it silently writes `.css`, which `--style=scss` prevents. Both generator defaults in `nx.json` are fixed here — `unitTestRunner` on `:application` and `:library` now reads `vitest-analog`, and `:library` gains `style: scss` — so the next generation needs neither flag. `@nx/angular:library` still has no way to skip the component it scaffolds; for an entity or util lib the whole `src/lib/` directory has to be deleted afterwards.
- **`libs/eco/entities` and `libs/eco-admin/core/{entities,tenant-context,layout-config}`**: only the four libraries AP-0 actually consumes, not the ~20-lib tree of REQUIREMENTS §7.3 — empty libs cost a lint and a test target each on every `nx affected` run. All four carry a placeholder type behind their alias rather than an empty barrel, because an empty `index.ts` behind a `tsconfig.base.json` path is a no-op for both lint and typecheck and would leave the new aliases untested. `layout-config` is `type:data-access`, not `type:util`: eco-admin's `VIEW_CONFIG` is role-partitioned **and** domain-conditional (DESIGN-BRIEF §3.10 — «Cicles» appears and disappears with the tenant's cycle mode), so it must read the tenant-context store, which `type:util → [type:util, type:entity]` forbids.
- **`.github/workflows/eco-admin-pa11y.yml`** and the `preeco-admin:a11y` / `eco-admin:a11y` / `eco-admin:a11y:run` script trio, plus `apps/eco-admin/.pa11yci.json`. `ci.yml` needed **no** edit at all: it is entirely `nx affected`, with `llecoop-firebase` as its single hard-coded project name, so lint, test and build pick the new app up on their own. The deploy workflow is deliberately **not** here — `admin.9botiga.top` has no host decided in any document, and REQUIREMENTS §5.2 forbids deploying eco-admin to any shared environment until the eco-store gate PRV-05b-A lands.

### Changed

- **`.eslintrc.json`**: `scope:eco` and `scope:eco-admin` join the `@nx/enforce-module-boundaries` matrix, and `scope:eco` is added to `scope:eco-store`'s allow-list (REQUIREMENTS §7.2 / AREF-08). `scope:eco` is a **new third tier** between `scope:shared` and the app scopes: the eco platform's shared domain layer, importable by eco-store and eco-admin and by nobody else. These rows had to ship with the scaffold rather than in a follow-up, because a tag with no matching `sourceTag` row is covered only by the `* → *` wildcard — it is _unenforced_, the exact opposite of the intent — and because the first promotion (AREF-01) would otherwise break eco-store's own lint the moment it imports from `libs/eco/entities`. Note that the seal against llecoop, nasa-images and plastikaweb needs no rule: their allow-lists are closed and simply never name `scope:eco`. Note also that **every library needs both a `scope:*` and a `type:*` tag** — the rules are ANDed and the `type:app` row lists no app scope, so a scope-only lib cannot be imported by any app.
- **`libs/shared/notification/entities/project.json`**: retagged `scope:core` → `scope:shared`, where it lives (AREF-08 housekeeping). Safe by construction: it is `type:entity`, and `scope:core`, `scope:shared` and every app scope permit both tags, so no existing importer changes.
- **`libs/core/table-with-filtering/project.json`**: `sourceRoot` pointed at `libs/core/layouts/feature/table-with-filtering/src`, a path that does not exist; corrected to `libs/core/table-with-filtering/src`.
- **`CLAUDE.md`**: the alias examples said `@plastikspace/...`, a prefix used **zero** times in the repo against 182 uses of `@plastik/` — corrected, along with the stale project counts (6 apps / 137 libs → 7 / 180) and the scope documentation, which now covers the product-family tier and the both-tags requirement.

### Removed

- **`apps/eco-admin-e2e/src/support/{commands.ts,app.po.ts}`**: the Cypress generator's placeholder `login` command and `getGreeting` page object. The former `console.log`ed a parameter named `password`, which CodeQL reports as `js/clear-text-logging` at **high** severity — not a false positive (the code really does log it) but with no exploitable impact, since it is an e2e support file that is never deployed and, in eco-admin, was never called. The fix is therefore deletion rather than dismissal: the alert closes at the root, and nobody later grows the real login command around a `console.log` of the credentials. `getGreeting` pointed at an `<h1>` the app does not render. Note that the identical boilerplate remains open as alerts #8-#11 on the other four `-e2e` apps; those are pre-existing and not addressed here, and `eco-store-e2e` additionally _calls_ `cy.login(...)`, so it needs the call removed too.

### Notes

The boundary rows are verified by a **negative lint test**, not by `lint --all`. With `libs/eco` still empty and nothing importing it, a full lint pass proves nothing about the new rows — a typo such as `scope:ecoadmin`, or the two allow-lists transposed, would go green. Adding a cross-scope `export * from '@plastik/eco-store/entities'` to `libs/eco-admin/core/entities` fails with the expected `@nx/enforce-module-boundaries` message; the permanent `eco-admin-tenant-context → eco-entities` edge covers the positive direction.

`AREF-09` is registered in REQUIREMENTS §7.2 and TASKS as **0.6b** — numbered to run _before_ the 0.7 shell, which is where the tokens are first needed. The shared M3/OKLCH token block that DESIGN-BRIEF treats as law lives inside `apps/eco-store/src/styles/eco-store-theme.css`, i.e. inside an application, so eco-admin ships no brand palette until it is promoted to `libs/eco/styles`. The trap that decision closes: `stylePreprocessorOptions.includePaths` is build configuration, not TypeScript imports, so pointing eco-admin at `apps/eco-store/src/styles` would work and would create app-to-app coupling that Nx boundaries cannot see.
