...
function createSelectorsFactory() {
  function getSelectors(selectState) {
    ...
    const selectAll = createSelector(selectIds, selectEntities, (ids, entities) => ids.map((id2) => entities[id2]));
...
function createUnsortedStateAdapter(selectId) {
  ...
  function updateManyMutably(updates, state) {
    ...
    if (didMutateEntities) {
      ...
      if (didMutateIds) {
        state.ids = state.ids.map((id2) => newKeys[id2] || id2);
        return DidMutate.Both;
      } else {
        return DidMutate.EntitiesOnly;
      }
  ...
  function mapMutably(map2, state) {
    const changes = state.ids.reduce((changes2, id2) => {
      const change = map2(state.entities[id2]);
      if (change !== state.entities[id2]) {
        changes2.push({ id: id2, changes: change });
      }
      return changes2;
    }, []);
    const updates = changes.filter(({ id: id2 }) => id2 in state.entities);
    return updateManyMutably(updates, state);
  }
  function mapOneMutably({ map: map2, id: id2 }, state) {
    const entity = state.entities[id2];
    if (!entity) {
      return DidMutate.None;
    }
...
function createSortedStateAdapter(selectId, sort) {
  ...
  function setOneMutably(entity, state) {
    const id2 = selectIdValue(entity, selectId);
    if (id2 in state.entities) {
      state.ids = state.ids.filter((val) => val !== id2);
      merge2([entity], state);
      return DidMutate.Both;
    } else {
  ...
  function updateManyMutably(updates, state) {
    ...
    if (models.length === 0) {
      ...
      state.ids = state.ids.filter((id2, index2) => {
        if (id2 in state.entities) {
          return true;
        } else {
          updatedIndexes.push(index2);
          return false;
        }
      });
      merge2(models, state);
  ...
  function mapMutably(updatesOrMap, state) {
    const updates = state.ids.reduce((changes, id2) => {
      const change = updatesOrMap(state.entities[id2]);
      if (change !== state.entities[id2]) {
        changes.push({ id: id2, changes: change });
      }
      return changes;
    }, []);
    return updateManyMutably(updates, state);
...
var selectNasaImagesFeature = createFeature({
  ...
  extraSelectors: ({ selectImagesState, selectEntities, selectSelectedId }) => {
    return __spreadProps(__spreadValues({}, nasaMediaAdapter.getSelectors(selectImagesState)), {
      selectSelectedEntity: createSelector(selectEntities, selectSelectedId, (entities, id2) => entities && id2 ? entities[id2] : null)
    });
  }
});
...
var HttpCrudService = class extends BaseDataService {
  ...
  getOne(id2, options) {
    void options;
    return this.httpClient.get(`${this.apiUrl}/${id2}`).pipe(map((response) => this.mapItemResponse(response)), catchError(this.handleError));
  }
  /**
   ...
   */
  update(id2, data, options) {
    void options;
    return this.httpClient.patch(`${this.apiUrl}/${id2}`, data).pipe(map((response) => this.mapItemResponse(response)), catchError(this.handleError));
  }
  /**
   ...
   */
  delete(id2) {
    return this.httpClient.delete(`${this.apiUrl}/${id2}`).pipe(map(() => true), catchError(this.handleError));
  }
  /**
...
var NasaImagesApiService = class _NasaImagesApiService extends HttpGetAllService {
  ...
  mapListResponse({ collection: { metadata: { total_hits: count }, items } }) {
    const mappedItems = items.map(({ data, links }) => {
      const { nasa_id: id2, title: name2, date_created, description: description2, keywords, center: center2, location, secondary_creator: creator2 } = data[0];
      return {
        id: id2,
        name: name2,
...
var NasaImagesEffects = class _NasaImagesEffects {
  ...
  showNotification$ = createEffect(() => {
    return this.#actions$.pipe(ofType(nasaImagesAPIActions.loadFailure), map(({ error }) => {
      const message2 = error || "The request has failed. Please try it again.";
      this.#liveAnnouncer.announce(message2, "assertive", 5e3);
...
function getFieldId(formId, field, index2) {
  if (field.id) {
    return field.id;
  }
  let type = field.type;
  ...
  return [formId, type, field.key, index2].join("_");
}
...
var CoreExtension = class {
  ...
  onPopulate(field) {
    ...
    if (field.fieldGroup) {
      field.fieldGroup.forEach((f, index2) => {
        if (f) {
...
var _FormlyConfig = class _FormlyConfig {
  ...
  getMergedField(field = {}) {
    ...
    if (field?.optionsTypes) {
      field.optionsTypes.forEach((option) => {
        const defaultOptions2 = this.getType(option).defaultOptions;
        if (defaultOptions2) {
          reverseDeepMerge(field, defaultOptions2);
        }
      });
    }
  ...
  addValidatorMessage(name2, message2) {
    this.messages[name2] = message2;
    if (typeof ngDevMode === "undefined" || ngDevMode) {
      ...
      if (deprecated[name2]) {
        ...
        this.messages[deprecated[name2]] = message2;
      }
    }
  }
...
var _LegacyFormlyTemplate = class _LegacyFormlyTemplate extends FormlyTemplate {
};
_LegacyFormlyTemplate.\u0275fac = /* @__PURE__ */ (() => {
...
_LegacyFormlyTemplate.\u0275dir = /* @__PURE__ */ \u0275\u0275defineDirective({
  type: _LegacyFormlyTemplate,
  selectors: [["", "formlyTemplate", ""]],
  standalone: false,
  features: [\u0275\u0275InheritDefinitionFeature]
});
var LegacyFormlyTemplate = _LegacyFormlyTemplate;
(() => {
...
var _FormlyAttributes = class _FormlyAttributes {
  ...
  detachElementRef(f) {
    const index2 = f?.["_elementRefs"] ? this.fieldAttrElements.indexOf(this.elementRef) : -1;
    if (index2 !== -1) {
      f["_elementRefs"].splice(index2, 1);
    }
  }
  ...
  get errorMessage() {
    ...
    for (const error in fieldForm.errors) {
      if (fieldForm.errors.hasOwnProperty(error)) {
        let message2 = this.config.getValidatorMessage(error);
        if (isObject(fieldForm.errors[error])) {
          if (fieldForm.errors[error].errorPath) {
            return void 0;
          }
          if (fieldForm.errors[error].message) {
            message2 = fieldForm.errors[error].message;
          }
        }
        if (this.field.validation?.messages?.[error]) {
          message2 = this.field.validation.messages[error];
        }
        if (this.field.validators?.[error]?.message) {
          message2 = this.field.validators[error].message;
        }
        if (this.field.asyncValidators?.[error]?.message) {
          message2 = this.field.asyncValidators[error].message;
        }
        if (typeof message2 === "function") {
          return message2(fieldForm.errors[error], this.field);
        }
        return message2;
      }
    }
...
var FormlyAddonsWrapperComponent = class _FormlyAddonsWrapperComponent extends FieldWrapper {
  matPrefix = viewChild("matPrefix", __spreadProps(__spreadValues({}, ngDevMode ? { debugName: "matPrefix" } : (
    /* istanbul ignore next */
    {}
  )), { read: TemplateRef }));
  matSuffix = viewChild("matSuffix", __spreadProps(__spreadValues({}, ngDevMode ? { debugName: "matSuffix" } : (
    /* istanbul ignore next */
    {}
  )), { read: TemplateRef }));
  constructor() {
...
function FormlyWrapperFormField_Conditional_4_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275property("ngTemplateOutlet", ctx_r0.props.textPrefix)("ngTemplateOutletContext", \u0275\u0275pureFunction1(2, _c03, ctx_r0.field));
  }
}
function FormlyWrapperFormField_Conditional_5_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275property("ngTemplateOutlet", ctx_r0.props.prefix)("ngTemplateOutletContext", \u0275\u0275pureFunction1(2, _c03, ctx_r0.field));
  }
}
function FormlyWrapperFormField_Conditional_6_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275property("ngTemplateOutlet", ctx_r0.props.textSuffix)("ngTemplateOutletContext", \u0275\u0275pureFunction1(2, _c03, ctx_r0.field));
  }
}
function FormlyWrapperFormField_Conditional_7_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275property("ngTemplateOutlet", ctx_r0.props.suffix)("ngTemplateOutletContext", \u0275\u0275pureFunction1(2, _c03, ctx_r0.field));
  }
}
function FormlyWrapperFormField_Conditional_10_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275property("ngTemplateOutlet", stringOrTemplate_r2)("ngTemplateOutletContext", \u0275\u0275pureFunction1(2, _c13, ctx));
  }
}
...
function FormlyWrapperFormField_ng_template_12_Conditional_1_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const content_r3 = \u0275\u0275nextContext().content;
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275property("ngTemplateOutlet", content_r3)("ngTemplateOutletContext", \u0275\u0275pureFunction1(2, _c03, ctx_r0.field));
  }
}
...
_FormlyWrapperFormField.\u0275fac = function FormlyWrapperFormField_Factory(__ngFactoryType__) {
...
var _FormlyMatFormFieldModule = class _FormlyMatFormFieldModule {
};
_FormlyMatFormFieldModule.\u0275fac = function FormlyMatFormFieldModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMatFormFieldModule)();
};
_FormlyMatFormFieldModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
  type: _FormlyMatFormFieldModule,
  declarations: [FormlyWrapperFormField],
  imports: [CommonModule, ReactiveFormsModule, MatFormFieldModule, FormlyModule]
});
_FormlyMatFormFieldModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
  imports: [CommonModule, ReactiveFormsModule, MatFormFieldModule, FormlyModule.forChild(withFormlyFormField())]
});
var FormlyMatFormFieldModule = _FormlyMatFormFieldModule;
(() => {
...
_FieldType2.\u0275fac = /* @__PURE__ */ (() => {
  let \u0275FieldType_BaseFactory;
  return function FieldType_Factory(__ngFactoryType__) {
    return (\u0275FieldType_BaseFactory || (\u0275FieldType_BaseFactory = \u0275\u0275getInheritedFactory(_FieldType2)))(__ngFactoryType__ || _FieldType2);
  };
})();
_FieldType2.\u0275dir = /* @__PURE__ */ \u0275\u0275defineDirective({
  ...
  viewQuery: function FieldType_Query(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275viewQuery(_c23, 5)(_c32, 5)(_c42, 5)(_c52, 5)(MatFormFieldControl, 5);
    }
    if (rf & 2) {
      let _t;
...
var _FormlyMatInputModule = class _FormlyMatInputModule {
};
_FormlyMatInputModule.\u0275fac = function FormlyMatInputModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMatInputModule)();
};
_FormlyMatInputModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
...
_FormlyMatInputModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
...
var FormlyMatInputModule = _FormlyMatInputModule;
(() => {
...
var _c04 = [[["", 8, "extra-links"]]];
var _c14 = [".extra-links"];
var SharedFormFeatureComponent = class _SharedFormFeatureComponent {
  ...
  formGroupDirective = viewChild(FormGroupDirective, ...ngDevMode ? [{ debugName: "formGroupDirective" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  onModelChange(model) {
    ...
    if (this.formSubmitConfig().emitOnChange && (this.form.valid || this.formSubmitConfig().emitInvalid)) {
  ...
  #resetFormStatus(forceReset = false) {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SharedFormFeatureComponent, selectors: [["plastik-shared-form-feature"]], viewQuery: function SharedFormFeatureComponent_Query(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275viewQuerySignal(ctx.formGroupDirective, FormGroupDirective, 5);
    }
    if (rf & 2) {
      \u0275\u0275queryAdvance();
    }
  }, hostAttrs: [1, "w-full"], inputs: { fields: [1, "fields"], model: [1, "model"], submitConfig: [1, "submitConfig"], autoFocus: [1, "autoFocus"], disableForm: [1, "disableForm"], resetForm: [1, "resetForm"] }, outputs: { changeEvent: "changeEvent", temporaryChangeEvent: "temporaryChangeEvent", pendingChangesEvent: "pendingChangesEvent", validChange: "validChange" }, ngContentSelectors: _c14, decls: 7, vars: 16, consts: [["novalidate", "", 1, "mt-4", "flex", "w-full", "flex-col", "gap-1", 3, "ngSubmit", "formGroup"], [1, "form-container", 3, "modelChange", "fields", "form", "model", "options"], ["mat-flat-button", "", "type", "submit", "data-test", "submit-button", "title", "Submit", 3, "disabled"]], template: function SharedFormFeatureComponent_Template(rf, ctx) {
    if (rf & 1) {
...
`, styles: ["/* libs/shared/form/feature/src/lib/shared-form-feature.component.scss */\n.shared-form--extraLinks-container ::ng-deep > .extraLinks {\n  font-size: 13px;\n  gap: var(--sub);\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  flex-direction: column;\n}\n/*# sourceMappingURL=shared-form-feature.component.css.map */\n"] }]
  }], () => [], { fields: [{ type: Input, args: [{ isSignal: true, alias: "fields", required: true }] }], model: [{ type: Input, args: [{ isSignal: true, alias: "model", required: false }] }], submitConfig: [{ type: Input, args: [{ isSignal: true, alias: "submitConfig", required: false }] }], autoFocus: [{ type: Input, args: [{ isSignal: true, alias: "autoFocus", required: false }] }], disableForm: [{ type: Input, args: [{ isSignal: true, alias: "disableForm", required: false }] }], resetForm: [{ type: Input, args: [{ isSignal: true, alias: "resetForm", required: false }] }], changeEvent: [{ type: Output, args: ["changeEvent"] }], temporaryChangeEvent: [{ type: Output, args: ["temporaryChangeEvent"] }], pendingChangesEvent: [{ type: Output, args: ["pendingChangesEvent"] }], validChange: [{ type: Output, args: ["validChange"] }], formGroupDirective: [{ type: ViewChild, args: [forwardRef(() => FormGroupDirective), { isSignal: true }] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SharedFormFeatureComponent, { className: "SharedFormFeatureComponent", filePath: "libs/shared/form/feature/src/lib/shared-form-feature.component.ts", lineNumber: 36 });
})();
...
var _FormlyMatTextAreaModule = class _FormlyMatTextAreaModule {
};
_FormlyMatTextAreaModule.\u0275fac = function FormlyMatTextAreaModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMatTextAreaModule)();
};
_FormlyMatTextAreaModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
  type: _FormlyMatTextAreaModule,
  declarations: [FormlyFieldTextArea],
  imports: [CommonModule, ReactiveFormsModule, MatInputModule, FormlyMatFormFieldModule, FormlyModule]
});
_FormlyMatTextAreaModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
...
var FormlyMatTextAreaModule = _FormlyMatTextAreaModule;
(() => {
...
var _FormlySelectOptionsPipe = class _FormlySelectOptionsPipe {
  ...
  transformOptions(options, field) {
    ...
    options?.forEach((option) => {
      ...
      if (o.group) {
        const id2 = groups[o.label];
        if (id2 === void 0) {
          groups[o.label] = opts.push(o) - 1;
        } else {
          o.group.forEach((o2) => opts[id2].group.push(o2));
        }
      } else {
        opts.push(o);
      }
...
var _FormlyMatRadioModule = class _FormlyMatRadioModule {
};
_FormlyMatRadioModule.\u0275fac = function FormlyMatRadioModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMatRadioModule)();
};
_FormlyMatRadioModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
...
_FormlyMatRadioModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
...
var FormlyMatRadioModule = _FormlyMatRadioModule;
(() => {
...
_FormlyFieldCheckbox.\u0275fac = function FormlyFieldCheckbox_Factory(__ngFactoryType__) {
...
var _FormlyMatCheckboxModule = class _FormlyMatCheckboxModule {
};
_FormlyMatCheckboxModule.\u0275fac = function FormlyMatCheckboxModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMatCheckboxModule)();
};
_FormlyMatCheckboxModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
...
_FormlyMatCheckboxModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
...
var FormlyMatCheckboxModule = _FormlyMatCheckboxModule;
(() => {
...
var _FormlyMatMultiCheckboxModule = class _FormlyMatMultiCheckboxModule {
};
_FormlyMatMultiCheckboxModule.\u0275fac = function FormlyMatMultiCheckboxModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMatMultiCheckboxModule)();
};
_FormlyMatMultiCheckboxModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
...
_FormlyMatMultiCheckboxModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
...
var FormlyMatMultiCheckboxModule = _FormlyMatMultiCheckboxModule;
(() => {
...
var _FormlyMatSelectModule = class _FormlyMatSelectModule {
};
_FormlyMatSelectModule.\u0275fac = function FormlyMatSelectModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMatSelectModule)();
};
_FormlyMatSelectModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
...
_FormlyMatSelectModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
...
var FormlyMatSelectModule = _FormlyMatSelectModule;
(() => {
...
var _FormlyMaterialModule = class _FormlyMaterialModule {
};
_FormlyMaterialModule.\u0275fac = function FormlyMaterialModule_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FormlyMaterialModule)();
};
_FormlyMaterialModule.\u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({
...
_FormlyMaterialModule.\u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({
  imports: [FormlyMatFormFieldModule, FormlyMatInputModule, FormlyMatTextAreaModule, FormlyMatRadioModule, FormlyMatCheckboxModule, FormlyMatMultiCheckboxModule, FormlyMatSelectModule]
});
var FormlyMaterialModule = _FormlyMaterialModule;
(() => {
...
var DateFnsAdapter = class _DateFnsAdapter extends DateAdapter {
  ...
  getMonthNames(style) {
    ...
    return range2(12, (i) => this.format(new Date(2017, i, 1), pattern));
  }
  getDateNames() {
  ...
  getDayOfWeekNames(style) {
    ...
    return range2(7, (i) => this.format(new Date(2017, 0, i + 1), pattern));
  }
  getYearName(date) {
...
var MatSingleDateSelectionModel = class _MatSingleDateSelectionModel extends MatDateSelectionModel {
  ...
  clone() {
    const clone2 = new _MatSingleDateSelectionModel(this._adapter);
    clone2.updateSelection(this.selection, this);
    return clone2;
  }
...
var MatRangeDateSelectionModel = class _MatRangeDateSelectionModel extends MatDateSelectionModel {
  ...
  clone() {
    const clone2 = new _MatRangeDateSelectionModel(this._adapter);
    clone2.updateSelection(this.selection, this);
    return clone2;
  }
...
var MatStartDate = class _MatStartDate extends MatDateRangeInputPartBase {
  ...
  _assignValueToModel(value) {
    if (this._model) {
      const range3 = new DateRange(value, this._model.selection.end);
...
var MatEndDate = class _MatEndDate extends MatDateRangeInputPartBase {
  ...
  _assignValueToModel(value) {
    if (this._model) {
      const range3 = new DateRange(this._model.selection.start, value);
      this._model.updateSelection(range3, this);
    }
  }
...
var YearPickerTypeComponent = class _YearPickerTypeComponent extends FieldType2 {
  static \u0275fac = /* @__PURE__ */ (() => {
...
var _c07 = (a0) => [a0];
var _c16 = (a0, a1, a2) => ({ priority: a0, w: a1, h: a2 });
var _c25 = (a0) => ({ priority: a0 });
var _c34 = (a0, a1) => ({ quality: a0, availableThumbs: a1 });
...
function SharedImgContainerComponent_Conditional_4_For_1_Conditional_3_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    const priority_r3 = \u0275\u0275readContextLet(2);
    \u0275\u0275nextContext();
    const fixedImage_r4 = \u0275\u0275reference(3);
    \u0275\u0275property("ngTemplateOutlet", fixedImage_r4)("ngTemplateOutletContext", \u0275\u0275pureFunction3(2, _c16, priority_r3, width_r1, height_r2));
  }
}
function SharedImgContainerComponent_Conditional_4_For_1_Conditional_4_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementContainer(0, 6);
  }
  if (rf & 2) {
    ...
    \u0275\u0275property("ngTemplateOutlet", fillImage_r5)("ngTemplateOutletContext", \u0275\u0275pureFunction1(2, _c25, priority_r3));
  }
}
function SharedImgContainerComponent_Conditional_4_For_1_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275declareLet(0)(1)(2);
    \u0275\u0275conditionalCreate(3, SharedImgContainerComponent_Conditional_4_For_1_Conditional_3_Template, 1, 6, "ng-container", 6)(4, SharedImgContainerComponent_Conditional_4_For_1_Conditional_4_Template, 1, 4, "ng-container", 6);
  }
  if (rf & 2) {
    const dim_r6 = ctx.$implicit;
    const ctx_r6 = \u0275\u0275nextContext(2);
    const width_r8 = \u0275\u0275storeLet(dim_r6 == null ? null : dim_r6.width);
    \u0275\u0275advance();
    const height_r9 = \u0275\u0275storeLet(dim_r6 == null ? null : dim_r6.height);
    \u0275\u0275advance();
    \u0275\u0275storeLet(ctx_r6.lcpImage());
    \u0275\u0275advance();
    \u0275\u0275conditional(width_r8 && height_r9 ? 3 : 4);
  }
}
function SharedImgContainerComponent_Conditional_4_ng_template_2_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275listener("load", function SharedImgContainerComponent_Conditional_4_ng_template_2_Template_img_load_0_listener() {
      \u0275\u0275restoreView(_r10);
      const ctx_r6 = \u0275\u0275nextContext(2);
      return \u0275\u0275resetView(ctx_r6.onImageLoad());
    })("error", function SharedImgContainerComponent_Conditional_4_ng_template_2_Template_img_error_0_listener() {
      \u0275\u0275restoreView(_r10);
      const ctx_r6 = \u0275\u0275nextContext(2);
      return \u0275\u0275resetView(ctx_r6.onImageError());
    });
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const priority_r11 = ctx.priority;
    const w_r12 = ctx.w;
    const h_r13 = ctx.h;
    const ctx_r6 = \u0275\u0275nextContext(2);
    const src_r14 = \u0275\u0275readContextLet(0);
    \u0275\u0275classMap(`object-cover transition-opacity duration-100 ${ctx_r6.imgClass()}`);
    \u0275\u0275classProp("opacity-0", ctx_r6.isLoading() && !ctx_r6.lcpImage())("opacity-100", !ctx_r6.isLoading() && !ctx_r6.hasError() || ctx_r6.lcpImage());
    \u0275\u0275property("priority", priority_r11)("ngSrc", src_r14)("width", w_r12)("height", h_r13)("alt", ctx_r6.title())("loaderParams", \u0275\u0275pureFunction2(13, _c34, ctx_r6.quality(), ctx_r6.thumbSizes()));
    \u0275\u0275attribute("fetchpriority", priority_r11 ? "high" : "low");
  }
}
function SharedImgContainerComponent_Conditional_4_ng_template_4_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const priority_r16 = ctx.priority;
    const ctx_r6 = \u0275\u0275nextContext(2);
    const src_r14 = \u0275\u0275readContextLet(0);
    \u0275\u0275classProp("opacity-0", ctx_r6.isLoading())("opacity-100", !ctx_r6.isLoading() && !ctx_r6.hasError());
    \u0275\u0275property("priority", priority_r16)("ngSrc", src_r14)("alt", ctx_r6.title())("sizes", ctx_r6.sizes())("ngSrcset", ctx_r6.computedSrcset())("loaderParams", \u0275\u0275pureFunction2(11, _c34, ctx_r6.quality(), ctx_r6.thumbSizes()));
    \u0275\u0275attribute("fetchpriority", priority_r16 ? "high" : "low");
  }
}
function SharedImgContainerComponent_Conditional_4_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275repeaterCreate(0, SharedImgContainerComponent_Conditional_4_For_1_Template, 5, 4, null, null, _forTrack02);
    \u0275\u0275template(2, SharedImgContainerComponent_Conditional_4_ng_template_2_Template, 1, 16, "ng-template", null, 0, \u0275\u0275templateRefExtractor)(4, SharedImgContainerComponent_Conditional_4_ng_template_4_Template, 1, 14, "ng-template", null, 1, \u0275\u0275templateRefExtractor);
  }
  if (rf & 2) {
    const ctx_r6 = \u0275\u0275nextContext();
    \u0275\u0275repeater(\u0275\u0275pureFunction1(0, _c07, ctx_r6.dimensions()));
  }
}
var SharedImgContainerComponent = class _SharedImgContainerComponent {
  ...
  imgClass = input("object-cover", ...ngDevMode ? [{ debugName: "imgClass" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SharedImgContainerComponent, selectors: [["plastik-img-container"]], hostAttrs: [1, "relative", "block", "overflow-hidden"], hostVars: 2, hostBindings: function SharedImgContainerComponent_HostBindings(rf, ctx) {
    ...
    if (rf & 2) {
      const src_r17 = \u0275\u0275storeLet(ctx.src());
      \u0275\u0275advance();
      \u0275\u0275classProp("bg-surface-container-high", ctx.isLoading())("breathing", ctx.isLoading());
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.isLoading() ? 2 : -1);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.hasError() ? 3 : -1);
      \u0275\u0275advance();
      \u0275\u0275conditional(src_r17 ? 4 : -1);
    }
...
<div
  ...
  [class.bg-surface-container-high]="isLoading()"
  ...
  @if (hasError()) {
    <div class="image-error bg-surface-container flex h-full w-full items-center justify-center">
...
@if (src) {
  ...
  @for (dim of [dimensions()]; track (dim?.width ?? 0) + '-' + (dim?.height ?? 0)) {
    ...
    @let priority = lcpImage();

    @if (width && height) {
      <ng-container
        [ngTemplateOutlet]="fixedImage"
        [ngTemplateOutletContext]="{ priority, w: width, h: height }"></ng-container>
    } @else {
      <ng-container
        [ngTemplateOutlet]="fillImage"
        [ngTemplateOutletContext]="{ priority }"></ng-container>
    }
  }

  <ng-template #fixedImage let-priority="priority" let-w="w" let-h="h">
    <img
      [class]="\`object-cover transition-opacity duration-100 \${imgClass()}\`"
...
var NasaImagesSearchFeatureTableConfig = class {
  static getTableDefinition() {
    ...
    return toSignal(store.select(selectRouteQueryParams).pipe(map(({ page = 0 }) => {
      return __spreadProps(__spreadValues({}, defaultTableConfig), {
        columnProperties,
        pagination: computed(() => __spreadProps(__spreadValues({}, defaultTableConfig.pagination || {}), {
          pageSize: 100,
          pageIndex: --page
        })),
        count,
...
var _c08 = () => [];
var _c17 = () => ({});
var _c26 = (a0, a1) => ({ data: a0, isActiveSearch: a1 });
function NasaImagesSearchFeatureComponent_Conditional_0_Defer_0_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1(" ", info_r1 == null ? null : info_r1.icon, " ");
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(info_r1 == null ? null : info_r1.title);
  }
}
function NasaImagesSearchFeatureComponent_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domTemplate(0, NasaImagesSearchFeatureComponent_Conditional_0_Defer_0_Template, 5, 2);
    \u0275\u0275defer(1, 0, NasaImagesSearchFeatureComponent_Conditional_0_Defer_1_DepsFn);
    \u0275\u0275deferOnIdle();
  }
}
function NasaImagesSearchFeatureComponent_Defer_3_Template(rf, ctx) {
  if (rf & 1) {
    const _r2 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "plastik-shared-form-feature", 3);
    \u0275\u0275pipe(1, "ngrxPush");
    \u0275\u0275pipe(2, "ngrxPush");
    \u0275\u0275listener("changeEvent", function NasaImagesSearchFeatureComponent_Defer_3_Template_plastik_shared_form_feature_changeEvent_0_listener($event) {
      \u0275\u0275restoreView(_r2);
      const ctx_r2 = \u0275\u0275nextContext();
  ...
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275property("fields", \u0275\u0275pipeBind1(1, 2, ctx_r2.formStructure$) || \u0275\u0275pureFunction0(6, _c08))("model", \u0275\u0275pipeBind1(2, 4, ctx_r2.formModel$) || \u0275\u0275pureFunction0(7, _c17));
  }
}
function NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
...
function NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
...
function NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_ng_template_3_Template(rf, ctx) {
}
function NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_Template(rf, ctx) {
  if (rf & 1) {
    const _r4 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "plastik-shared-table", 5);
    \u0275\u0275listener("changePagination", function NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_Template_plastik_shared_table_changePagination_0_listener($event) {
      \u0275\u0275restoreView(_r4);
      const ctx_r2 = \u0275\u0275nextContext(2);
      return \u0275\u0275resetView(ctx_r2.onChangePagination($event));
    });
    \u0275\u0275conditionalCreate(1, NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_Conditional_1_Template, 7, 0, "plastik-nasa-images-search-ui-no-results", 6);
    \u0275\u0275conditionalCreate(2, NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_Conditional_2_Template, 9, 0, "plastik-nasa-images-search-ui-no-results", 0);
    \u0275\u0275template(3, NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_ng_template_3_Template, 0, 0, "ng-template", null, 0, \u0275\u0275templateRefExtractor);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const obs_r5 = ctx.ngrxLet;
    \u0275\u0275nextContext();
    const table_r6 = \u0275\u0275readContextLet(0);
    \u0275\u0275property("data", obs_r5.data || \u0275\u0275pureFunction0(7, _c08))("caption", (table_r6 == null ? null : table_r6.caption) || "")("columnProperties", (table_r6 == null ? null : table_r6.columnProperties()) || \u0275\u0275pureFunction0(8, _c08))("resultsLength", (table_r6 == null ? null : table_r6.count()) || 0)("pagination", table_r6 == null ? null : table_r6.pagination());
    \u0275\u0275advance();
...
function NasaImagesSearchFeatureComponent_Defer_6_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275declareLet(0);
    \u0275\u0275template(1, NasaImagesSearchFeatureComponent_Defer_6_plastik_shared_table_1_Template, 5, 9, "plastik-shared-table", 4);
  }
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275storeLet(ctx_r2.tableDefinition());
    \u0275\u0275advance();
    \u0275\u0275property("ngrxLet", \u0275\u0275pureFunction2(2, _c26, ctx_r2.images$, ctx_r2.isActiveSearch$));
  }
}
var NasaImagesSearchFeatureComponent = class _NasaImagesSearchFeatureComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _NasaImagesSearchFeatureComponent, selectors: [["plastik-nasa-images-search"]], decls: 9, vars: 3, consts: [["noValidSearch", ""], ["data-test", "page-title", 1, "gap-sm", "h3", "mb-4", "flex", "items-center"], [1, "scale-150"], [3, "changeEvent", "fields", "model"], [3, "data", "caption", "columnProperties", "resultsLength", "pagination", "changePagination", 4, "ngrxLet"], [3, "changePagination", "data", "caption", "columnProperties", "resultsLength", "pagination"], ["noResults", ""], ["icon", ""], ["title", ""], ["message", ""]], template: function NasaImagesSearchFeatureComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275conditionalCreate(0, NasaImagesSearchFeatureComponent_Conditional_0_Template, 3, 0);
      \u0275\u0275pipe(1, "ngrxPush");
      \u0275\u0275elementStart(2, "section");
      \u0275\u0275domTemplate(3, NasaImagesSearchFeatureComponent_Defer_3_Template, 3, 8);
      \u0275\u0275defer(4, 3, NasaImagesSearchFeatureComponent_Defer_4_DepsFn);
      \u0275\u0275deferOnIdle();
      \u0275\u0275domTemplate(6, NasaImagesSearchFeatureComponent_Defer_6_Template, 2, 5);
      \u0275\u0275defer(7, 6, NasaImagesSearchFeatureComponent_Defer_7_DepsFn);
      \u0275\u0275deferOnIdle();
      \u0275\u0275elementEnd();
    }
    if (rf & 2) {
...
//# sourceMappingURL=src-ZE4U4NV3.js.map
