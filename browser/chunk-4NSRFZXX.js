...
} from "./chunk-NCFKKHWY.js";
import {
  MatIcon,
  MatIconModule
} from "./chunk-L7VWDKOV.js";
import {
  MatButton,
  MatButtonModule,
  MatIconButton,
...
function createSortedStateAdapter(selectId, sort) {
  ...
  function updateManyMutably(updates, state) {
    ...
    if (models.length === 0) {
      ...
      merge2(models, state);
      if (!didMutateIds && updatedIndexes.every((i2) => state.ids[i2] === originalIds[i2])) {
        return DidMutate.EntitiesOnly;
      } else {
        return DidMutate.Both;
      }
  ...
  function merge2(models, state) {
    ...
    let i2 = 0;
    let j = 0;
    while (i2 < models.length && j < state.ids.length) {
    ...
    if (i2 < models.length) {
      state.ids = ids.concat(models.slice(i2).map(selectId));
    } else {
      state.ids = ids.concat(state.ids.slice(j));
    }
    models.forEach((model, i3) => {
...
var ApiService = class _ApiService extends BaseDataService {
  ...
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ApiService, factory: _ApiService.\u0275fac });
};
(() => {
...
var PocketBaseService = class _PocketBaseService extends BaseDataService {
  #pb;
  /**
  ...
  constructor() {
    super();
    this.#pb = new Client(this.getPocketBaseUrlFromEnvironment());
  }
  /**
   ...
   */
  getList(params) {
    return from(this.#pb.collection(this.collectionName()).getList(params?.page || 1, params?.perPage || 50, params || {})).pipe(map((data) => this.mapListResponse(data)), shareReplay({ bufferSize: 1, refCount: true, windowTime: this.cacheTime }), catchError(this.handleError));
  }
  /**
   ...
   */
  getFullList(params) {
    return from(this.#pb.collection(this.collectionName()).getFullList(params)).pipe(map((items) => items.map((item) => this.mapResponse(item))), shareReplay({ bufferSize: 1, refCount: true, windowTime: this.cacheTime }), catchError(this.handleError));
  }
  /**
   ...
   */
  getOne(id, options) {
    return from(this.#pb.collection(this.collectionName()).getOne(id, options)).pipe(map((data) => this.mapResponse(data)), shareReplay({ bufferSize: 1, refCount: true, windowTime: this.cacheTime }), catchError(this.handleError));
  }
  /**
   ...
   */
  getFirstListItem(filter2, options) {
    return from(this.#pb.collection(this.collectionName()).getFirstListItem(filter2, options)).pipe(map((data) => this.mapResponse(data)), shareReplay({ bufferSize: 1, refCount: true, windowTime: this.cacheTime }), catchError(this.handleError));
  }
  /**
   ...
   */
  create(data, options) {
    return from(this.#pb.collection(this.collectionName()).create(data, options)).pipe(map((response) => this.mapResponse(response)), catchError(this.handleError));
  }
  /**
   * @param { string } id The record ID.
   * @param { Partial<T> } data The record data.
   ...
   */
  update(id, data, options) {
    return from(this.#pb.collection(this.collectionName()).update(id, data, options)).pipe(map((response) => this.mapResponse(response)), catchError(this.handleError));
  }
  /**
   * @param { string } id The record ID.
   ...
   */
  delete(id) {
    return from(this.#pb.collection(this.collectionName()).delete(id)).pipe(map(() => true), catchError(this.handleError));
  }
  static \u0275fac = function PocketBaseService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _PocketBaseService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _PocketBaseService, factory: _PocketBaseService.\u0275fac });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(PocketBaseService, [{
    type: Injectable
  }], () => [], null);
})();
...
var NasaImagesApiService = class _NasaImagesApiService extends ApiService {
  resourceUrlSegment() {
    return "search";
  }
...
function observe(o, paths, setFn) {
  ...
  let target = o;
  for (let i2 = 0; i2 < paths.length - 1; i2++) {
    if (!target[paths[i2]] || !isObject(target[paths[i2]])) {
      target[paths[i2]] = /^\d+$/.test(paths[i2 + 1]) ? [] : {};
    }
    target = target[paths[i2]];
  }
...
function getField(f, key) {
  ...
  for (let i2 = 0, len = f.fieldGroup.length; i2 < len; i2++) {
    const c = f.fieldGroup[i2];
    ...
    if (hasKey(field) || !field.parent || !hasKey(field) && !field.fieldGroup) {
      ...
      if (c) {
        ...
        if (markForCheck) {
          ...
          let parent = c.parent;
          for (let i2 = 1; i2 < getKeyPath(field).length; i2++) {
            if (parent) {
  ...
  resolveFieldTypeRef(field = {}) {
    ...
    if (!this.componentRefs[type.name]) {
      ...
      try {
        ...
        console.error(`An error occurred while destroying the Formly component type "${field.type}"`, e2);
      }
    }
  ...
  triggerHook(name2, changes) {
    ...
    if (this.field?.hooks?.[name2]) {
      if (!changes || changes.field) {
        const r2 = this.field.hooks[name2](this.field);
        if (isObservable(r2) && ["onInit", "afterContentInit", "afterViewInit"].indexOf(name2) !== -1) {
          const sub = r2.subscribe();
...
var _FormlyAttributes = class _FormlyAttributes {
    ...
    if (changes.field) {
      ...
      this.uiEvents.events.forEach((eventName) => {
        if (this.props?.[eventName] || ["focus", "blur", "change"].indexOf(eventName) !== -1) {
          this.uiEvents.listeners.push(this.renderer.listen(this.elementRef.nativeElement, eventName, (e2) => this.uiEvents.callback(eventName, e2)));
        }
      });
  ...
  ngDoCheck() {
    ...
    for (let i2 = 0; i2 < this.uiAttributes.length; i2++) {
      const attr = this.uiAttributes[i2];
...
var _FieldArrayType = class _FieldArrayType extends FieldType {
  onPopulate(field) {
    ...
    if (field.fieldGroup.length > length) {
      for (let i2 = field.fieldGroup.length - 1; i2 >= length; --i2) {
        unregisterControl(field.fieldGroup[i2], true);
        field.fieldGroup.splice(i2, 1);
      }
    }
    for (let i2 = field.fieldGroup.length; i2 < length; i2++) {
  ...
  add(i2, initialModel, {
    ...
    i2 = i2 == null ? this.field.fieldGroup.length : i2;
    if (!this.model) {
      assignFieldValue(this.field, []);
    }
    this.model.splice(i2, 0, initialModel ? clone(initialModel) : void 0);
    this.markFieldForCheck(this.field.fieldGroup[i2]);
    this._build();
  }
  remove(i2, {
    markAsDirty
    ...
    markAsDirty && this.formControl.markAsDirty();
    this.model.splice(i2, 1);
    const field = this.field.fieldGroup[i2];
    this.field.fieldGroup.splice(i2, 1);
  ...
  updateArrayElementKey(f, newKey) {
    ...
    for (let i2 = 0; i2 < f.fieldGroup.length; i2++) {
      this.updateArrayElementKey(f.fieldGroup[i2], newKey);
    }
  }
  markFieldForCheck(f) {
    if (!f) {
...
var FormlyAddonsWrapperComponent = class _FormlyAddonsWrapperComponent extends FieldWrapper {
  matPrefix = viewChild("matPrefix", ...ngDevMode ? [{ debugName: "matPrefix", read: TemplateRef }] : [{ read: TemplateRef }]);
  matSuffix = viewChild("matSuffix", ...ngDevMode ? [{ debugName: "matSuffix", read: TemplateRef }] : [{ read: TemplateRef }]);
  constructor() {
    super();
    effect(() => {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _FormlyAddonsWrapperComponent, selectors: [["plastik-formly-addons-wrapper"]], viewQuery: function FormlyAddonsWrapperComponent_Query(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275viewQuerySignal(ctx.matPrefix, _c02, 5, TemplateRef);
      \u0275\u0275viewQuerySignal(ctx.matSuffix, _c12, 5, TemplateRef);
    }
    if (rf & 2) {
      \u0275\u0275queryAdvance(2);
    }
...
var _c13 = [".extraLinks"];
function SharedFormFeatureComponent_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275property("ngClass", ctx_r0.config().buttonStyle || "")("disabled", !ctx_r0.form.valid);
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", ctx_r0.config().label || "Submit", " ");
  }
}
var SharedFormFeatureComponent = class _SharedFormFeatureComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SharedFormFeatureComponent, selectors: [["plastik-shared-form-feature"]], hostAttrs: [1, "w-full"], inputs: { fields: [1, "fields"], model: [1, "model"], submitConfig: [1, "submitConfig"], autoFocus: [1, "autoFocus"], disableForm: [1, "disableForm"] }, outputs: { changeEvent: "changeEvent", temporaryChangeEvent: "temporaryChangeEvent", pendingChangesEvent: "pendingChangesEvent" }, ngContentSelectors: _c13, decls: 5, vars: 6, consts: [["novalidate", "", 1, "flex", "flex-col", "w-full", "gap-sub", 3, "ngSubmit", "formGroup"], [1, "form-container", 3, "modelChange", "form", "options", "fields", "model"], ["mat-flat-button", "", "type", "submit", "data-test", "submit-button", 1, "w-full", "sm:w-auto", 3, "ngClass", "disabled"], [1, "flex", "justify-center", "p-0", "text-base", "shared-form--extraLinks-container"]], template: function SharedFormFeatureComponent_Template(rf, ctx) {
    if (rf & 1) {
      ...
      \u0275\u0275elementEnd();
      \u0275\u0275conditionalCreate(2, SharedFormFeatureComponent_Conditional_2_Template, 2, 3, "button", 2);
    ...
    if (rf & 2) {
      ...
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.config().submitAvailable ? 2 : -1);
    }
  }, dependencies: [ReactiveFormsModule, \u0275NgNoValidate, NgControlStatusGroup, FormGroupDirective, FormlyModule, LegacyFormlyForm, MatButtonModule, MatButton, MatIconModule, NgClass], styles: ["\n\n.shared-form--extraLinks-container[_ngcontent-%COMP%]     > .extraLinks {\n  font-size: 13px;\n  gap: var(--sub);\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  flex-direction: column;\n}\n/*# sourceMappingURL=shared-form-feature.component.css.map */"], changeDetection: 0 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SharedFormFeatureComponent, [{
    type: Component,
    args: [{ selector: "plastik-shared-form-feature", imports: [ReactiveFormsModule, FormlyModule, MatButtonModule, MatIconModule, NgClass], host: {
      class: "w-full"
...
var MatFormField = class _MatFormField {
  ...
  _prefixSuffixContainers = computed(() => {
    return [this._iconPrefixContainerSignal(), this._textPrefixContainerSignal(), this._iconSuffixContainerSignal(), this._textSuffixContainerSignal()].map((container) => container?.nativeElement).filter((e2) => e2 !== void 0);
  }, ...ngDevMode ? [{
...
function FormlyWrapperFormField_ng_template_12_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275conditionalCreate(0, FormlyWrapperFormField_ng_template_12_Conditional_0_Template, 1, 1)(1, FormlyWrapperFormField_ng_template_12_Conditional_1_Template, 1, 4, "ng-container", 10);
  }
  if (rf & 2) {
...
var VERSION3 = new Version("20.2.10");
...
@angular/forms/fesm2022/forms.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-4NSRFZXX.js.map
